...
for file in star_path.rglob('*'):
    if '.git' not in str(file) and '__' not in str(file) and 'version.json' not in str(file) and '.gitignore' not in str(file) and 'LICENSE' not in str(file) and 'star_list.json' not in str(file) and 'env' not in str(file) and 'logs' not in str(file) and 'Auto_Star_Rail_backup' not in str(file) and os.path.isfile(file):
        star_list.append({
